...
NFT_OWNER = 'tz1LPjwnqwXNG8QXpXSGRanARGVf8sRoLh6m'

AUTH_NAME = 'john.newby'
AUTH_PASS = #secret
...
async def process_nfts(nfts):
    ...
    try:
        ...
        # print('\n')
        # print(new_mark_minted(nft['id'], ipfs=metadata_ipfs_hash))
...
async def poll():
    while True:
        try:
            r = requests.get(
                'https://human-machine.io/api/artwork/?status=ready', auth=(AUTH_NAME, AUTH_PASS))
...
if __name__ == '__main__':
    ...
    # asyncio.get_event_loop().run_until_complete(process_nfts(nfts['results']))
